...
public class MultiColumnRecrawlDataSchema extends RecrawlDataSchemaBase implements RecrawlDataSchema, RecrawlAttributeConstants {
    ...
    public void load(Result result, CrawlURI curi) {
        ...
        // URL.
        byte[] nocrawl = result.getValue(COLUMN_FAMILY, COLUMN_NOCRAWL);
        ...
        byte[] status = result.getValue(COLUMN_FAMILY, COLUMN_STATUS);
        if (status != null) {
            ...
            byte[] etag = result.getValue(COLUMN_FAMILY, COLUMN_ETAG);
            if (etag != null) {
                history.put(RecrawlAttributeConstants.A_ETAG_HEADER, Bytes.toString(etag));
            }
            byte[] lastmod = result.getValue(COLUMN_FAMILY, COLUMN_LAST_MODIFIED);
            if (lastmod != null) {
            ...
            byte[] digest = result.getValue(COLUMN_FAMILY, COLUMN_CONTENT_DIGEST);
            if (digest != null) {
